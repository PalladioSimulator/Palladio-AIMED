...
if(is.character(nbins))
	...
	if(plot) {
		...
		invisible(list(heights = heights, xbr = xbr,counts=counts,breaks=xbr))
	}
	else {
		return(list(heights = heights, xbr = xbr,counts=counts,breaks=xbr))
	}
}

doublePDF<-function(myvector)
{
    # Create histogram
    myhist <- dhist(myvector)
    
    # Calculate probabilities
    myprobabilities <- myhist$counts/sum(myhist$counts)
    myprobabilities <- append(myprobabilities, 0.0, 0)
    myprobabilities <- as.vector(myprobabilities)

    # Create DoublePDF
    mylist <- list(myhist$breaks, myprobabilities)
    tuples <- list()
    for ( i in (1:length(mylist[[1]])) )
    {
        tuples <- append(tuples, paste("(", paste(mylist[[1]][i], paste(";", paste(mylist[[2]][i], ")", sep = ""), sep = ""), sep = ""), sep = "") )
    }
    tuples <- gsub("e", "E", tuples)
    result <- paste("DoublePDF[", paste(paste(tuples, collapse = ""), "]", sep = ""), sep = "")
